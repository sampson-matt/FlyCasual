        ...
        private bool ValidateStandardizedCards(SquadList squad)
        {
            ...
            foreach (var standardizedPair in standardizedUpgradesFound)
            {
                foreach (var shipConfig in squad.Ships)
                {
                    ...
                    {
                        if (!shipConfig.Instance.UpgradeBar.HasUpgradeInstalled(standardizedPair.Value.GetType()) && shipConfig.Instance.UpgradeBar.HasUpgradeSlot(standardizedPair.Value.Slot.Type) && shipConfig.Instance.PilotInfo.AffectedByStandardized)
                        {
