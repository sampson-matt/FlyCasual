...
namespace Abilities.SecondEdition
{
    public class TapuskAbility : GenericAbility
    {
        ...
        private void ChooseAbility()
        {
            ...
            if(TargetShip.State.RegensCharges > 0 && TargetShip.State.Charges < TargetShip.State.MaxCharges && TargetShip.State.Force < TargetShip.State.MaxForce)
            {
        ...
        public List<GenericShip> GetAvailableTargets()
        {
            return BoardTools.Board.GetShipsAtRange(HostShip,  new UnityEngine.Vector2(0, 3), Team.Type.Enemy)
                .Where(n => HostShip.ArcsInfo.HasShipInTurretArc(n) && ((n.State.RegensCharges >0 && n.State.Charges < n.State.MaxCharges) || n.State.Force < n.State.MaxForce)).ToList();
        }
